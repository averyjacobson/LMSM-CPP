...
void asm_parse_src(compilation_result * result, char * original_src){
    ...
    while (token != NULL) {
        ...
        char *label = NULL; //is token in instruction database
        if (!asm_is_instruction(token)) { //its a label
            label = token;
            token = strtok(NULL," \n");
        }
        ...
        if (asm_instruction_requires_arg(instruction)) {
            ...
            if (token != NULL) {
                if (asm_is_num(token)) {
                    if (atoi(token) > 999 || atoi(token) < -999) {
        ...
        //HINT last instruction to create linked list w/ current instruction
        last_instruction = current_instruction; //set previous instruction
        token = strtok(NULL, " \n");
    }
